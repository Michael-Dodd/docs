...
description: "This guide will walk you through the process of setting up a Store Finder in JavaScript, using the Loqate Store Finder APIs and MapLibre for map rendering."
sidebarTitle: "How To Implement a Store Finder"
---
...
<Note>Note that this guide is specific to setting up a Store Finder using JavaScript, which is the only supported integration currently. <br/><br/>As mentioned above this guide focuses on using MapLibre for rendering, however other open source libraries are available for this - such as Leaflet and Tanagram.</Note>

<Tip>**CSS Styling** <br/><br/>Tailwind CSS styling is used throughout this guide to minimise the need for 'custom' styling, however you can use whatever CSS styling you like.</Tip>
...
## Prerequisites
Before getting started it is essential that you understand some of the key concepts of a Store Finder and have everything set up to start building.

You should:

- Read the [How Does a Store Finder Work guide](/our-services/store-finder/how-does-a-store-finder-work), which explains some of the core concepts behind our APIs
- [Create a Store Finder service in your account](/our-services/store-finder/setting-up-store-finder-keys), which will enable the appropriate keys to access our APIs
- Create a Location List via either [the UI](/our-services/store-finder/creating-and-managing-location-lists) or our [Create List API](/api-reference/store-finder/location-management-create-list)
...
![Our initial web page](/images/store-finder/how-to-implement-a-store-finder/store-finder-how-screen-1.png)
...
async function initMap(key) {
  try {
    ...
    // Optionally: Rethrow the error after logging it, or handle it in a way that’s appropriate for your user experience.
    // throw error;
  }
}
...
![Our page with a map added](/images/store-finder/how-to-implement-a-store-finder/store-finder-how-screen-2.png)

## Adding geocoding
The next step is to add a way of capturing your end user's address/postcode/city/locality and converting it into co-ordinates. Co-ordinates are required for us to do our distance calculation later in the guide.

The following code will add a simple input field, a Find button to start the geocode process, and a marker to the map of where the end user is.
...
3. Save the file.
4. Open index.js and add the following additions. The first code block wraps the call to our [Geocoding API](/api-reference/store-finder/global-geocoding), while the second block is a helper function for adding markers to our map.
...
5. Add the following to the main function. This adds an event handler to the Find button that will call our Geocoding API and then use the co-ordinates returned to add a marker to the map and move to that location.
...
6. View what we have so far. You can type an address into the text box and click Find - the map should then update with the location you entered pinned on the map.

![Displaying a location on the map](/images/store-finder/how-to-implement-a-store-finder/store-finder-how-screen-3.png)

## Displaying store information and markers
This next step is to retrieve the closest stores to the end user and display these in the left pane, along with markers on the map. This requires the use of a previously uploaded Location List either via our [Create List API](/api-reference/store-finder/location-management-create-list) or the [List Upload UI](/our-services/store-finder/creating-and-managing-location-lists) in your Account section. 
...
2. Then add the following as the last element within the body section. This determines which pieces of store information will be displayed on the page.
...
3. Open index.js and add the following, which sets the parameters for the call to our [Distance Finder API](/api-reference/store-finder/global-distance-finder), taking an origin location (where your end user is), the listID of your stores and other parameters such as the max distance.
...
4. Add the following to index.js to add your store information to the left pane, and add a marker representing the store to the map.
...
5. Update **main** to the following. This will make the call to our Distance Finder API and iterate through the results to display them in the UI and on the map. **Remember to Update the locationListId variable with the desired Id of the uploaded list.**
...
6. You can now test by entering an address in the text box and clicking Find. This will update the map with locations close to the address entered.

![A working Store Finder](/images/store-finder/how-to-implement-a-store-finder/store-finder-how-screen-4.png)

## Attribution Statement
You must ensure attribution is visible when rendering a map as part of your application. Loqate uses map styling from [OpenMapTiles](https://osmfoundation.org/wiki/Licence/Attribution_Guidelines) and open data from [OpenStreetMaps](https://www.openstreetmap.org/copyright).

Attribution is provided by default as part of the map style configuration, however if you choose to create your own map styling you must still ensure that this attribution is visible. For further information on copyright notices please see the [Loqate Terms and Conditions](https://www.gbgplc.com/en/legal-and-regulatory/products/loqate-storefinder/legal-notices-loqate-storefinder/). 